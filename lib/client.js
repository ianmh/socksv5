...
function Client(options) {
  ...
  if (typeof this._proxyaddr !== 'string') this._proxyaddr = 'localhost';
  else if (typeof this._proxyport !== 'number') this._proxyport = 1080;

  this._dstaddr = undefined;
  this._dstport = undefined;
  this._localDNS =
    options && typeof options.localDNS === 'boolean' ? options.localDNS : true;
  this._strictLocalDNS =
    options && typeof options.strictLocalDNS === 'boolean'
...
Client.prototype._onConnect = function () {
  ...
  authsbuf[0] = 0x05;
  authsbuf[1] = alen;
  for (var a = 0, p = 2; a < alen; ++a, ++p) authsbuf[p] = auths[a].METHOD;
        ...
        if (auths[i].METHOD === method) {
          auths[i].client(socket, function (result) {
            if (result === true) {
              ...
              self._hadError = true;
              if (util.isError(result)) self.emit('error', result);
              else {
...
Client.prototype._sendRequest = function () {
  ...
  var addrlen =
      iptype === 0 ? Buffer.byteLength(self._dstaddr) : iptype === 4 ? 4 : 16,
  ...
  if (iptype > 0) {
    ...
    p = 4;
    for (var i = 0; i < addrlen; ++i, ++p) reqbuf[p] = addrbytes[i];
  } else {
    reqbuf[3] = ATYP.NAME;
    reqbuf[4] = addrlen;
...
Client.prototype.connect = function (options, cb) {
  ...
  if (!options.port) throw new Error('Can only connect to TCP hosts');
  ...
  if (this._parser) this._parser.stop();
  ...
  if (net.isIP(this._dstaddr) === 0 && this._localDNS) {
    dns.lookup(this._dstaddr, function (err, addr) {
      ...
      if (addr) self._dstaddr = addr;
      self._sock.connect(realOptions);
    });
  } else this._sock.connect(realOptions);
